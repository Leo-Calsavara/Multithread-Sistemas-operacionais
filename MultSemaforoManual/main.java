...
class Motorista {
    ...
    public void aceitarCorrida(String nome_cliente, Corrida corrida) {
      ...
      if (disponivel && !corrida.mutex_flag && !corrida.corrida_ja_executada){// Se o motorista estiver diponivel and a corrida não esta no meio da execução and a corrida ainda não foi executada...
          try {
            this.disponivel = false; // Motorista não mais disponivel
            corrida.mutex_flag = true; // Corrida já em Exec
...
class Gerenciador {
	...
	public void ChamaMotoristas(String nome_cliente, Corrida corrida) {
        Thread th1 = new Thread(() -> vetorMotorista[0].aceitarCorrida(nome_cliente, corrida)); // Cada pedido de corrida vira uma thread
